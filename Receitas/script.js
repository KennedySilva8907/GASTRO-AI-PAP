/**
 * Objeto que contém as informações de todas as receitas
 * Cada receita possui um título e o conteúdo HTML
 */
const recipes = {
    bacalhau_a_bras: {
...
/**
 * Classe principal para gerenciar o carrossel vertical
 * Controla a navegação, animações e interações do usuário
 */
class VerticalScrollCarousel {
    /**
     * Inicializa o carrossel com opções personalizáveis
     * @param {Object} options - Configurações opcionais para o carrossel
     */
    constructor(options = {}) {
    ...
    preloadImages() {
        ...
        bgImages.forEach((bgImg) => {
            const imgUrl = this.getBackgroundImageUrl(bgImg);
    ...
    /**
     * Extrai a URL da imagem de fundo de um elemento
     * @param {HTMLElement} element - Elemento com background-image
    ...
    preloadNextImages(currentIndex) {
        ...
        for (let i = 1; i <= 2; i++) {
            ...
            const imgUrl = this.getBackgroundImageUrl(bgImages[nextIndex]);
    ...
    setupModalEventListeners() {
        ...
        this.openModal = (videoId, recipeKey) => {
            ...
            if (recipes[recipeKey]) {
                ...
                recipeTitle.textContent = recipes[recipeKey].title;
                recipeContent.innerHTML = recipes[recipeKey].content;
...
/**
 * Classe para gerenciar o comportamento responsivo do modal
 * Lida com gestos de toque, redimensionamento e acessibilidade
 */
...
/**
 * Inicialização do carrossel e do handler responsivo quando o DOM estiver carregado
 */
document.addEventListener('DOMContentLoaded', () => {
    new VerticalScrollCarousel();
    new ResponsiveModalHandler();
});

/**
 * Detecção de dispositivos móveis e otimizações específicas
 */
...
window.addEventListener('load', () => {
    ...
    // Desativa animações durante redimensionamento
    let resizeTimer;
    window.addEventListener('resize', () => {
...
/**
 * Funcionalidade do botão de voltar com animação de transição
 */
